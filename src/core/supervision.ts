 ...
 * 自激防护完全交给 LLM 判断（prompt 里告知"最后一条是自己时如何抉择"）；
 * 不再用「最后说话人是自己 → 自动跳过」的硬 guard，避免误伤 PM 发完无 @ 总结后流程真卡死的情况。
 */
interface BotInfo {
  profileName: string
  displayName: string
...
2. **系统不再自动跳过"自己刚说过话"的轮次**——下次 tick 你会再次面对该局面，需由你**主动判断**是否继续介入
3. 切忌陷入"自答自"死循环：若 1 轮自答后无新的外部输入，下次 tick 应输出 shouldFollowUp=false 等待外部消息
...
### 关于"最近消息最后一条就是你自己（${selfProfile}）"的判断

由于已删除自动跳过保护，你**可能会**看到最后一条就是你自己上一轮发的。这种局面要分情况：
- 若那条**已 @ 了某个 agent** → 给对方留响应时间，**应输出 shouldFollowUp: false**（避免重复打扰）
- 若那条是**无 @ 的总结/感叹/"完成"声明**，但暗含"下一步该谁做"的指令 → 群已实际卡住，**应派对应 agent 把那个"下一步"接住**
- 若最近你已连续 2 条以上发言、且无外部新输入 → 流程已停在你自己嘴里，**应输出 shouldFollowUp: false** 等外部信号
- 若你那条只是闲聊/打招呼/无任何下一步含义 → 群已自然结束，**应输出 shouldFollowUp: false**

#### 信号词 → 执行者 映射（用来辨识"暗含下一步"）

| 你上一条出现的措辞 | 含义 | 怎么办 |
|---|---|---|
| "进入 X 阶段" / "开始 X" / "下一步是 X" | 暗含执行 X | 从 agent 列表里找 X 的执行者，@ TA |
| "需要 Y 评审 / 实施 / 测试" | 暗含 Y 角色介入 | @ Y 对应的 agent |
| "PR 已起 / 工单已立 / 任务已派" | 暗含被指派人接力 | @ 后端/相关执行 agent |
| "等甲方 / 等用户 / 等外部反馈" | 外部输入 | shouldFollowUp: false |
| "完成" / "搞定" 单独出现，无下文 | 自然收束 | shouldFollowUp: false |
| "OK" / "好的" / "收到" | 闲聊确认 | shouldFollowUp: false |

#### 若决定要继续推进，reason 字段必须显式写明推理链

凡是判定 shouldFollowUp=true 且上一条是你自己时，**reason 字段必须包含两项内容**：
1. 你从上一条里提取出的"暗含的下一步动作"是什么
2. 该动作的执行者 profileName 是谁、为何选 TA

然后：
- mention 字段填该执行者 profileName
- message 字段用**祈使句**给出具体可执行任务，避免空泛
  - ✘ 空泛："继续 PR-I2" / "推进一下"
  - ✓ 具体："请实施 PR-I2 的 routing 模块，参考 09:45 评审里的 X 决议"
