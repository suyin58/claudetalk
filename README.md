...
2）以 Claude Code SubAgent 作为每个角色的执行引擎，支持为不同 Agent 独立配置 Prompt 和模型； 
...
- **角色即主循环**：profile 的 `systemPrompt` 通过 `--append-system-prompt` 注入到 Claude Code 主循环，机器人本身就是该角色（不通过 SubAgent 调度，回复就是本人口吻）
- **可选模型覆盖**：profile 配置里加 `model` 字段会透传给 `claude --model`，让不同 bot 跑不同模型（如 PM 用 Haiku 省成本，Dev 用 Sonnet 保质量）
...
每个 profile 启动后是一个独立 Claude Code 主循环进程，角色身份通过 CLI 参数注入：

| 字段 | 注入方式 | 作用 |
|------|---------|------|
| `systemPrompt` | `claude --append-system-prompt <prompt>` | 把角色定义追加到主循环的 system prompt，主循环本身就是该角色，直接以本人口吻回复 |
| `model`（可选） | `claude --model <id>` | 指定该 profile 用的模型，不填走 `claude` CLI 默认 |
...
|---|---|---|
| feishu | `<at user_id="ou_xxx">显示名</at>` （bot 优先 openId，user 用 unionId） | peer-message 路由按 appId 主匹配 + name 模糊兜底 |
...
| Channel | @ 格式 | 路由匹配 |
|---------|--------|------|
| 飞书 | `<at user_id="ou_xxx">机器人名称</at>` | peer-message 路由按 `appId` 主匹配 → `name` 模糊兜底；user_id 用 openId 因为 feishu API 只认 ou_/on_ 前缀 |
