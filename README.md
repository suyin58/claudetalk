...
> 目前仅飞书 Channel 支持图片输入，钉钉暂不支持。

飞书机器人支持接收**文字**、**图片**以及**文字 + 图片混合**的消息，并将图片传递给 Claude 进行分析。
...
|---------|------|
| 文字 | 直接传给 Claude 处理 |
| 图片 | 下载到本地后，告知 Claude 图片路径，由 Claude 读取分析 |
| 文字 + 图片（富文本） | 提取文字和图片，一并传给 Claude |
| 语音、视频、文件等 | 回复"暂不支持该类型"，不调用 Claude |

**纯图片消息的特殊处理**：

用户只发图片（不带文字）时，机器人不会立即调用 Claude，而是将图片缓存并等待后续指令：

```
用户发送一张图片
  → 🖼️ 图片已收到（共 1 张），请继续发送指令。

用户发送"帮我分析这张图"
  → Claude 收到：帮我分析这张图 + 图片路径，进行分析并回复
```

支持连续发多张图片后再发指令，图片会按发送顺序全部传给 Claude。群聊中不同用户的图片缓存相互独立，不会混淆。

图片文件保存在工作目录的 `.claudetalk/feishu/images/` 下，可按需清理。
