...
**默认行为**：每 20 分钟轮询、最后一条消息距今超 10 分钟视为停滞、介入后该群 20 分钟内不再次介入。
...
- @ 校验：`mention` 必须在群内已知 bot 列表中，`message` 必须含字面 `@<mention>` 字符串；校验失败重试一次再 drop
- LLM 调用 60s 超时，超时即 drop 本群（防止单次卡死让监督瘫痪）
...
| `~/.claudetalk/agents/{profile}.md` | 全局 SubAgent 定义文件（`--setup auto --global` 时写到此处；启动时本地优先回退全局） |
